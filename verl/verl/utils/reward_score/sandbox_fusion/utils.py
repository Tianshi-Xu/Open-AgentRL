...
"""
    ...
    if error_msg:
        ...
        logger.info(f"Case {case_index}: API error occurred: {error_msg}")
        # Log code and input only on error for brevity
        generation_to_log = generation[:200] + "..." if len(generation) > 200 else generation
        logger.info(f"Case {case_index}: code: {generation_to_log}")
        logger.info(f"Case {case_index}: input: {stdin}")
    elif api_response:
        # --- Add debug logging ---
