...
class RayPPOTrainer:
    ...
    def _filter_zero_advantage_samples(self, batch: DataProto):
        ...
        IMPORTANT: This filters entire groups (all N responses for a prompt), not individual samples,
        to maintain the group structure required by GRPO. Groups are identified by uid.
        ...
        # Group samples by uid (GRPO groups by uid, not by sequential index)
        ...
        for uid, indices in uid_to_indices.items():
            # Sum advantages for all samples in this group
        ...
        print(f"Filtered {n_groups_filtered}/{n_groups_total} groups (kept {n_groups_kept})")
        
        # If all groups are filtered, keep at least one group to avoid empty batch
        if n_groups_kept == 0:
            print("Warning: All groups have zero advantage. Keeping the first group to avoid empty batch.")
            # Keep the first group (by uid)
        ...
        if mini_batch_size is not None:
            # Collect all valid groups (both filtered groups and unfiltered groups)
            valid_groups = []
            for uid, indices in uid_to_indices.items():
                if all(valid_sample_mask[idx] for idx in indices):
                    valid_groups.append((uid, indices))
            
            # If current n_valid is less than mini_batch_size, try to add more groups
            if n_valid < mini_batch_size:
                print(
                    ...
                    f"Adding back filtered groups to meet mini_batch_size requirement."
                )
                
                # Collect filtered groups (those with zero advantage)
                filtered_groups = []
                for uid, indices in uid_to_indices.items():
                    if not any(valid_sample_mask[idx] for idx in indices):
                        filtered_groups.append((uid, indices))
                
                # Add filtered groups back until we have at least mini_batch_size samples
                ...
                if n_valid < mini_batch_size:
                    print(
                        f"Warning: Even after adding back filtered groups, only {n_valid} samples remain, "
                        f"which is still less than mini_batch_size={mini_batch_size}. "
                        f"Keeping all {n_valid} samples."
                    )
            ...
            # Now adjust to make n_valid divisible by mini_batch_size
            if n_valid % mini_batch_size != 0:
                ...
                if n_to_keep < mini_batch_size:
                    ...
                    print(
                        f"Warning: Cannot make batch divisible by mini_batch_size={mini_batch_size}. "
                        f"Keeping all {n_valid} samples."
                    )
                else:
                    # Drop entire groups (not individual samples) to reach n_to_keep
                    ...
                    # Keep groups until we reach n_to_keep samples
                    ...
                    print(
                        f"Adjusted to keep {groups_kept_count} groups ({n_valid} samples) "
                        f"to be divisible by mini_batch_size={mini_batch_size}"
                    )
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    print(f"Generation time: {time.perf_counter() - time_start:.2f} seconds",flush=True)
                    time_start = time.perf_counter()
                    ...
                    print(f"Batch preparation time: {time.perf_counter() - time_start:.2f} seconds",flush=True)
                    time_start = time.perf_counter()
                    ...
                    # print("reward_tensor",reward_tensor[reward_tensor!=0])
                    print("total reward in this step:",reward_tensor.sum(),flush=True)
                    print(f"Reward computation time: {time.perf_counter() - time_start:.2f} seconds",flush=True)
                    time_start = time.perf_counter()
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        # print("advantages",batch.batch["advantages"].mean(dim=-1))
                        # Filter out samples with all-zero advantages if enabled
                        if self.config.trainer.get("filter_zero_advantage_samples", False):
                    ...
                    print(f"Advantage computation time: {time.perf_counter() - time_start:.2f} seconds",flush=True)
                    time_start = time.perf_counter()
                    ...
                    print(f"Model update time: {time.perf_counter() - time_start:.2f} seconds",flush=True)
                # validate
                if (
